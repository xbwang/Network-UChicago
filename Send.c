...
test(const char *host, const char *service)
{
	...
	buf = "GET /index.html HTTP/1.0\nConnection: Keep-Alive User-Agent: Mozilla/2.02Gold (WinNT; I) Host: www.ora.com Accept: image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, */*\n\n";
	//(void) write(s, buf, strlen(buf));
	(void) send(s, buf, strlen(buf), 0);
	/*
